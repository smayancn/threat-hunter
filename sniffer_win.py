...
def capture_network_details(interface, capture_time=5):
    ...
    try:
        ...
        if os.name == 'nt':  # Windows
            return capture_with_scapy(interface, capture_time)
        else:  # Linux/Unix
            return capture_with_socket(interface, capture_time)
    ...
    try:
        ...
        with open(all_packets_filename, 'w', newline='') as all_file, \
            ...
            def process_packet(packet):
                nonlocal total_packets, packet_count, tcp_count, tcp_with_flags, arp_count
                ...
                # Handle IP packets
                    ...
                    # Write IP packet to both CSV files
                    row_data = [
                        timestamp, src_mac_str, dest_mac_str, src_ip, dest_ip, 
                        protocol, packet_length, src_port, dest_port, tcp_flags, packet_direction
                    ]
...
def capture_with_socket(interface, capture_time=5):
    """Use raw sockets for packet capture on Linux systems"""
    try:
        # Create a raw socket (Linux only)
